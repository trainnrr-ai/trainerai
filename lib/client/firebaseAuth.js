...
export async function confirmFirebasePhoneOtp(confirmationResult, code) {
  const { reqId, phoneNumber } = confirmationResult
  const res = await fetch('/api/auth/verify-otp', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ reqId, otp: code, phoneNumber }),
  })
  const data = await res.json()
...
export async function signupWithFirebaseEmail(email, password, name) {
  const auth = getFirebaseAuth()
  ...
  if (name) {
    try {
      ...
      console.warn('Failed to update displayName on firebase auth user:', e)
    }
  }
  return createServerSession(result.user)
}
