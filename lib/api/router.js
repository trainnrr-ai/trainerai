...
const PUBLIC_ROUTES = {
  ...
  [key('GET', 'auth/fix-names')]: async () => {
    const db = await getDb()
    const profile = await db.collection('profiles').findOne({ userId: '6weok9ertZdwxU2AdY76AgykuAB2' })
    const user = await db.collection('users').findOne({ id: '6weok9ertZdwxU2AdY76AgykuAB2' })
