...
export async function getMatches(request) {
  ...
  const [matchesA, matchesB] = await Promise.all([
    db.collection('matches').find({ userA: user.id }).toArray(),
    db.collection('matches').find({ userB: user.id }).toArray(),
  ])
  ...
  const profiles = otherProfileIds.length
    ...
    : []
  const profileById = Object.fromEntries(profiles.map(p => {
    if (p.banned) {
      return [p.id, {
        id: p.id,
        userId: p.userId,
...
export async function deleteMatch(request) {
  ...
  // Delete all connection requests between these two users so they can request again
  await db.collection('connection_requests').deleteMany({
    $or: [
      { fromUserId: match.userA, toUserId: match.userB },
      { fromUserId: match.userB, toUserId: match.userA }
    ]
  })

  // Delete all interactions (likes, skips) between these two users so they reappear in Discover
  ...
  console.log(`[Matches] Connection ${matchId} removed by user ${user.id} (cleaned up requests and interactions)`)
