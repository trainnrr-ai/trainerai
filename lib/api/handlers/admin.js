...
export async function getAdminUsers(request) {
  ...
  if (q) {
    ...
    filter.$or = [{ email: rx }, { name: rx }, { phoneNumber: rx }]
  }
  ...
  // Enrich with profile info so admin sees onboarding status and can toggle verification
  const ids = users.map(u => u.id).filter(Boolean)
  const profiles = ids.length
    ? await db.collection('profiles').find({ userId: { $in: ids } }, { projection: { userId: 1, id: 1, name: 1, photos: 1, verified: 1, _id: 0 } }).toArray()
    : []
  const profileMap = Object.fromEntries(profiles.map(p => [p.userId, p]))
  const enriched = users.map(u => {
    const prof = profileMap[u.id] || null
    return {
      ...u,
      hasProfile: !!prof,
      profileId: prof?.id || null,
      profileName: prof?.name || null,
      profilePicture: prof?.photos?.[0] || null,
...
export async function getAdminAnalytics(request) {
  ...
  const isFirebase = (process.env.DATABASE_DRIVER || '').toLowerCase() === 'firebase'
  ...
  if (isFirebase) {
    ...
    const [allUsers, allProfiles, allMatches, allMessages] = await Promise.all([
      db.collection('users').find({ createdAt: { $gte: since } }).toArray(),
      db.collection('profiles').find({ createdAt: { $gte: since }, isSeed: { $ne: true } }).toArray(),
      db.collection('matches').find({ createdAt: { $gte: since } }).toArray(),
      db.collection('messages').find({ createdAt: { $gte: since } }).toArray(),
    ])

    const groupDocs = (docs, dateField = 'createdAt') => {
      const counts = {}
      for (const doc of docs) {
        const key = formatDate(doc[dateField])
    ...
    const profilesForGyms = await db.collection('profiles').find({ isSeed: { $ne: true } }).toArray()
    const gymCounts = {}
    for (const p of profilesForGyms) {
      if (p.gymName) gymCounts[p.gymName] = (gymCounts[p.gymName] || 0) + 1
    }
    ...
    const goalSplit = Object.entries(goalCounts)
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count)

    return NextResponse.json({
      days,
      signups: groupDocs(allUsers),
      profiles: groupDocs(allProfiles),
      matches: groupDocs(allMatches),
      messages: groupDocs(allMessages),
      topGyms,
      genderSplit,
      goalSplit,
    })
...
// Agent 1 - Admin Verify Fixed: manual verify/unverify endpoint
export async function postAdminVerifyProfile(request) {
  const adminUser = await getUserFromRequest(request)
  const db = await getDb()
  const { profileId, verified } = await request.json()
  if (!profileId) return jsonError('profileId required', 400)
  if (typeof verified !== 'boolean') return jsonError('verified must be true or false', 400)

  const profile = await db.collection('profiles').findOne({ id: profileId })
  if (!profile) return jsonError('Profile not found', 404)

  const update = { verified, updatedAt: new Date() }
  if (verified) {
    update.verifiedAt = new Date()
    update.verifiedBy = adminUser?.email || 'admin'
  } else {
    update.verifiedAt = null
    update.verifiedBy = null
  }
  await db.collection('profiles').updateOne({ id: profileId }, { $set: update })
  ...
  console.log(`[Admin] Profile ${profileId} ${verified ? 'verified' : 'unverified'} by ${adminUser?.email}`)
