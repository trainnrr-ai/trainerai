...
export async function upsertProfile(request) {
  ...
  // Check total size of base64 photos to stay under Firestore's 1MB doc limit
  const totalPhotoBytes = photos.reduce((sum, p) => {
  ...
  if (totalPhotoBytes > 750 * 1024) {
    return jsonError('Photos are too large. Please use smaller or fewer photos. Total must be under 750KB.', 400)
  }
  ...
  const seen = new Set()
  goals = goals.filter(g => (seen.has(g) ? false : (seen.add(g), true)))
  ...
  if (!body.name?.trim()) return jsonError('Please enter your name', 400)
  if (!body.age) return jsonError('Please enter your age', 400)
  if (!body.gender) return jsonError('Please select your gender identity', 400)
  if (!body.city?.trim()) return jsonError('Please specify your city or location', 400)
  if (!body.gymName?.trim()) return jsonError('Please enter your primary gym or workout location', 400)
  if (!body.level) return jsonError('Please select your current fitness experience level', 400)
  if (!body.timing) return jsonError('Please select your preferred workout timing window', 400)
...
export async function postVerifyFace(request) {
  const db = await getDb()
  const user = await getUserFromRequest(request)
  if (!user) return jsonError('Unauthorized', 401)
  
  const body = await request.json()
  const matchScore = typeof body.matchScore === 'number' ? body.matchScore : 100
