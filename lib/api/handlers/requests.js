...
export async function postDeclineRequest(request) {
  ...
  // Delete the connection request completely so they can request again
  await db.collection('connection_requests').deleteOne({ id: requestId })

  // Delete all interactions between these two users so they both reappear in each other's Discover
...
export async function getConnectionExclusions(db, userId) {
  ...
  const [sent, received, declined, activeMatches] = await Promise.all([
    db.collection('connection_requests')
      .find({ fromUserId: userId, status: 'pending' }, { projection: { toUserId: 1, _id: 0 } })
      .toArray(),
    db.collection('connection_requests')
      .find({ toUserId: userId, status: 'pending' }, { projection: { fromUserId: 1, _id: 0 } })
      .toArray(),
    db.collection('connection_requests')
  ...
  for (const r of sent) sentUserIds.add(r.toUserId)
  for (const r of received) sentUserIds.add(r.fromUserId)
  for (const m of activeMatches) {
    sentUserIds.add(m.userA === userId ? m.userB : m.userA)
  }
  return {
    sentUserIds,
