...
export async function getConnectionExclusions(db, userId) {
  ...
  const [sent, received, declined] = await Promise.all([
    db.collection('connection_requests')
      .find({ fromUserId: userId, status: { $in: ['pending', 'accepted'] } }, { projection: { toUserId: 1, _id: 0 } })
      .toArray(),
    db.collection('connection_requests')
      .find({ toUserId: userId, status: { $in: ['pending', 'accepted'] } }, { projection: { fromUserId: 1, _id: 0 } })
      .toArray(),
    db.collection('connection_requests')
