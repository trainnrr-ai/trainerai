...
export async function getDiscover(request) {
  ...
  // PROBLEM 4 — PROFILE ELIGIBILITY AUDIT
  const totalInDb = await db.collection('profiles').countDocuments()
  const nonSeedCount = await db.collection('profiles').countDocuments({ isSeed: { $ne: true } })
  const onboardingCompletedCount = await db.collection('profiles').countDocuments({ onboardingCompleted: true })
  console.log('[DISCOVER] Profile Eligibility Audit:')
  console.log('  - Total documents in profiles collection:', totalInDb)
  console.log('  - Profiles where isSeed is not true:', nonSeedCount)
  console.log('  - Profiles where onboardingCompleted is true:', onboardingCompletedCount)
  ...
  const query = {
    ...
    isSeed: { $ne: true },
    name: { $not: /Legacy|Test|Seed/i },
    photos: { $not: /pexels\.com/ },
  }
  
  // PROBLEM 1 — FILTER FIXES
  if (filters.city && filters.city.trim() !== '') query.city = filters.city
  ...
  console.log('[DISCOVER] Total profiles in DB (no filter):', totalInDb)
