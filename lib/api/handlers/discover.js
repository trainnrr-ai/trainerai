...
export async function getDiscover(request) {
  ...
  // Only apply city filter if explicitly specified and non-empty (supports wildcard 'Any' city queries)
  ...
  if (user) {
    ...
    // Always exclude the logged-in user's own userId from Discover results
    excludeUserIds.push(user.id)

    // Auto-heal legacy database states for connection removals (deletes orphaned accepted requests/likes)
    const [pendingRequests, activeMatches] = await Promise.all([
      db.collection('connection_requests').find({
    ...
    const activeUserIds = new Set([
      ...pendingRequests.map(r => r.fromUserId === user.id ? r.toUserId : r.fromUserId),
      ...activeMatches.map(m => m.userA === user.id ? m.userB : m.userA)
    ])

    // Clean up accepted requests with no match
    await db.collection('connection_requests').deleteMany({
      $or: [{ fromUserId: user.id }, { toUserId: user.id }],
      status: 'accepted',
      matchId: { $nin: activeMatches.map(m => m.id).filter(Boolean) }
    })

    // Clean up legacy 'like' interactions that are not active matches or pending requests
    const likes = await db.collection('interactions')
      .find({ fromUserId: user.id, action: 'like' })
      .toArray()
    const orphanedLikes = likes.filter(l => l.toUserId && !activeUserIds.has(l.toUserId))
    if (orphanedLikes.length > 0) {
      const orphanedProfileIds = orphanedLikes.map(l => l.toProfileId).filter(Boolean)
      await db.collection('interactions').deleteMany({
        fromUserId: user.id,
        toProfileId: { $in: orphanedProfileIds },
        action: 'like'
      })
    }
    ...
    excludeUserIds.push(...(conn?.sentUserIds || []), ...(conn?.declinedUserIds || []))
  }
  ...
  // Only surface discoverable profiles (must satisfy completion gate) and exclude banned ones
  profiles = profiles.filter(p => isProfileDiscoverable(p) && p.banned !== true)
