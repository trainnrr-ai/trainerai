...
export async function postAuthSendOtp(request) {
  try {
    ...
    if (!r.ok) {
      ...
      const widgetHash = crypto.createHash('sha256').update(widgetId).digest('hex')
      console.error('[postAuthSendOtp] MSG91 returned status:', r.status, errMsg)
      return jsonError(`${errMsg} (Token Hash: ${tokenHash}, Widget Hash: ${widgetHash}, Phone: ${cleanPhone})`, 500)
    }
