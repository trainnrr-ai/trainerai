...
import { verifyFirebaseIdToken, createFirebaseCustomToken } from '@/lib/firebase'
...
export async function postAuthSendOtp(request) {
  try {
    ...
    console.log('[postAuthSendOtp] Sending OTP via MSG91 for phone:', cleanPhone)
    ...
    if (!r.ok) {
      ...
      const maskedToken = authToken.length > 8 ? authToken.substring(0, 4) + '...' + authToken.substring(authToken.length - 4) : 'INVALID_LEN'
      const maskedWidget = widgetId.length > 8 ? widgetId.substring(0, 4) + '...' + widgetId.substring(widgetId.length - 4) : 'INVALID_LEN'
      console.error('[postAuthSendOtp] MSG91 returned status:', r.status, errMsg)
      return jsonError(`${errMsg} (Token: ${maskedToken}, Widget: ${maskedWidget}, Phone: ${cleanPhone})`, 500)
    }
...
export async function postAuthVerifyOtp(request) {
  try {
    ...
    console.log('[postAuthVerifyOtp] Verifying OTP via MSG91 for reqId:', reqId)
    ...
    if (!r.ok) {
      let errMsg = 'OTP verification request failed'
      try {
        const errData = await r.json()
        errMsg = `MSG91 Verify Error: ${errData.error || errData.message || JSON.stringify(errData)}`
      } catch (e) {
        try {
          const errText = await r.text()
          errMsg = `MSG91 Verify Error (${r.status}): ${errText}`
        } catch (e2) {}
      }
      console.error('[postAuthVerifyOtp] MSG91 verify failed with status:', r.status, errMsg)
    ...
    const cleanPhone = '+' + phoneNumber.replace(/\+/g, '').trim()
    let user = await db.collection('users').findOne({ phoneNumber: cleanPhone })
