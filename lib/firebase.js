...
function addFirestoreFilters(ref, query = {}) {
  ...
  if (equalities.length > 0) {
    ...
    // Push inequalities only if we have NO equalities (to bypass composite index requirements)
    const ineqFields = [...new Set(inequalities.map(x => x.field))]
    if (ineqFields.length === 1) {
      for (const ineq of inequalities) {
        q = q.where(ineq.field, ineq.op, ineq.val)
        pushed = true
      }
...
function isFullyPushable(query) {
  if (!query || typeof query !== 'object') return true
  if (query.$or || query.$and) return false
  
  let hasEqualities = false
  const inequalityFields = new Set()
  
  for (const [field, expected] of Object.entries(query)) {
    if (field.startsWith('$')) return false
    
    const isEq = !expected || typeof expected !== 'object' || expected instanceof Date || Array.isArray(expected) || expected.$in != null
    if (isEq) {
      hasEqualities = true
    } else {
      if (expected.$regex) return false
      if (expected.$ne !== undefined || expected.$gte != null || expected.$lte != null || expected.$gt != null || expected.$lt != null || expected.$nin != null) {
        inequalityFields.add(field)
      }
    }
  }
  
  if (hasEqualities && inequalityFields.size > 0) return false
...
class FirebaseCollection {
  ...
  async countDocuments(filter = {}) {
    if (isFullyPushable(filter)) {
      try {
        const { ref } = addFirestoreFilters(this.ref, filter)
        const snap = await ref.count().get()
        return snap.data().count
      } catch (err) {
        console.warn('[Firebase] Server-side count failed, falling back to client-side:', err.message)
      }
    }
  ...
  aggregate(pipeline = []) {
    ...
    if (firstStage && firstStage.$match) {
      queryPromise = this.filtered(firstStage.$match).then(items => runPipeline(items, pipeline.slice(1)))
    } else {
      queryPromise = this.all().then(items => runPipeline(items, pipeline))
    }
    return { toArray: async () => queryPromise }
  }
}
...
export async function createFirebaseCustomToken(uid, developerClaims) {
  initAdmin()
  return admin.auth().createCustomToken(uid, developerClaims)
}

export async function getOrCreateFirebaseUserByPhone(phoneNumber) {
  initAdmin()
  try {
