...
function addFirestoreFilters(ref, query = {}) {
  ...
  for (const [field, expected] of Object.entries(query)) {
    ...
    if (Array.isArray(expected.$nin) && expected.$nin.length > 0 && expected.$nin.length <= 10) {
      if (canPushInequality(field)) {
        q = q.where(field, 'not-in', expected.$nin)
        pushed = true
      }
      continue
    }
    if (expected.$gte != null) {
      if (canPushInequality(field)) {
        q = q.where(field, '>=', expected.$gte)
        pushed = true
      }
    }
    if (expected.$lte != null) {
      if (canPushInequality(field)) {
        q = q.where(field, '<=', expected.$lte)
        pushed = true
      }
    }
    if (expected.$gt != null) {
      if (canPushInequality(field)) {
        q = q.where(field, '>', expected.$gt)
        pushed = true
      }
    }
    if (expected.$lt != null) {
      if (canPushInequality(field)) {
        q = q.where(field, '<', expected.$lt)
        pushed = true
      }
    }
  }
