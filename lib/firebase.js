...
function addFirestoreFilters(ref, query = {}) {
  ...
  // Push inequalities if they are all on the same field (Firestore limitation: only 1 inequality field allowed)
  ...
  } else if (inequalities.length > 0 && equalities.length === 0) {
    // Fallback: push first inequality
...
function isFullyPushable(query) {
  ...
  for (const [field, expected] of Object.entries(query)) {
    ...
    if (expected && typeof expected === 'object' && !(expected instanceof Date) && !Array.isArray(expected)) {
      ...
      if (expected.$ne !== undefined || expected.$gte != null || expected.$lte != null || expected.$gt != null || expected.$lt != null) {
