...
class FirebaseCollection {
  ...
  async findOne(query = {}, options = {}) {
    ...
    const found = docs.find(d => matchesQuery(d, query))
    return found ? projectDoc(found, options.projection) : null
  }
  find(query = {}, options = {}) {
    const promise = this.all().then(docs => docs.filter(d => matchesQuery(d, query)).map(d => projectDoc(d, options.projection)))
    return {
  ...
  async updateOne(filter, update, options = {}) {
    const docs = await this.all()
    const found = docs.find(d => matchesQuery(d, filter))
  ...
  async updateMany(filter, update) {
    const docs = (await this.all()).filter(d => matchesQuery(d, filter))
  ...
  async countDocuments(filter = {}) {
    return (await this.all()).filter(d => matchesQuery(d, filter)).length
  }
  aggregate(pipeline = []) {
...
export async function getFirebaseDb() {
  ...
  return { collection: (name) => new FirebaseCollection(firestore, name) }
}
