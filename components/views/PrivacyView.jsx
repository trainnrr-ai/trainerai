...
function Section({ title, children }) {
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
      ...
      <div className="mt-2 text-sm text-slate-500 leading-relaxed space-y-2 font-semibold">{children}</div>
    </div>
  )
}
...
export default function PrivacyView({ onNav }) {
  return (
    <PageShell title="Privacy Policy" kicker="Last updated: 2025" onNav={onNav}>
      <p className="text-slate-650 font-semibold leading-relaxed">Trainr (trainr.in) is committed to protecting your privacy and building a safe fitness community. This page explains what we collect, how we use it, and how we keep you safe.</p>
      ...
      <div className="grid gap-4 mt-8">
        ...
        <Section title="Your data, your control">
          <p>You can edit, hide or delete your profile anytime from Settings. To request full account deletion, email <a href={`mailto:${SUPPORT_EMAIL}`} className="text-sky-600 hover:text-sky-700 font-bold underline">{SUPPORT_EMAIL}</a>.</p>
        </Section>
        <Section title="Contact">
          <p>For privacy questions, write to <a href={`mailto:${SUPPORT_EMAIL}`} className="text-sky-600 hover:text-sky-700 font-bold underline">{SUPPORT_EMAIL}</a> or DM us on Instagram <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="text-sky-600 hover:text-sky-700 font-bold underline">@trainr.in</a>.</p>
        </Section>
      </div>
    </PageShell>
  )
